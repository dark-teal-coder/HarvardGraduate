---
title: 'Challenge Problem: Rose Curves Solutions'
output:
...
```{r}
theta.sequence <-
  seq( from = 0, to = pi, length.out = 1000 )
```


```{r}
x.vector <-
  cos( 3 * theta.sequence ) * cos( theta.sequence )
```


```{r}
y.vector <-
  cos( 3 * theta.sequence ) * sin( theta.sequence )
```
...
```{r}
theta.sequence <-
  seq( from = 0, to = 6 * pi, length.out = 1000 )
```

```{r}
x.vector <-
  cos( 4/3 * theta.sequence ) * cos( theta.sequence )
```


```{r}
y.vector <-
  cos( 4/3 * theta.sequence ) * sin( theta.sequence )
```
...
```{r}
theta.sequence <-
  seq( from = 0, to = 7 * pi, length.out = 1000 )
```

```{r}
x.vector <-
  cos( 3/7 * theta.sequence ) * cos( theta.sequence )
```


```{r}
y.vector <-
  cos( 3/7 * theta.sequence ) * sin( theta.sequence )
```
